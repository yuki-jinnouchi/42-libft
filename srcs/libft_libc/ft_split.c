...
/*   Updated: 2023/11/27 16:38:45 by yjinnouc         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
unsigned int	strlen_c(char const *s, char c)
{
	int	count;
...
char	*word_capture(char const *s, char c, int n)
{
	char			*ptr;
	...
	s_count--;
	ptr = malloc(sizeof(char) * (strlen_c(s + s_count, c) + 1));
	if (!ptr)
		return (0);
	w_count = 0;
...
char	**ft_split(char const *s, char c)
{
	...
	{
		arr[count] = word_capture(s, c, count + 1);
		if (arr[count] == 0)
			return (arr_free(arr, count--));
		count++;
	}
	arr[count] = 0;
	return (arr);
